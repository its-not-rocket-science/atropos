...
def write_svg(path: Path, title: str, body: str) -> None:
    title_text = (
        ...
        f'font-size="20" font-family="Arial">{title}</text>'
    )
    path.write_text(
        "\n".join(
            [
...
def scale(
    values: Iterable[float], lo: float, hi: float, pixel_lo: float, pixel_hi: float
) -> list[float]:
    vals = list(values)
    if hi == lo:
...
def scatter_pred_vs_actual(rows: list[dict[str, str]], output: Path) -> None:
    pts = [(to_float(r.get("pred_memory_gb")), to_float(r.get("actual_memory_gb"))) for r in rows]
    pts = [(x, y) for x, y in pts if x is not None and y is not None]

    if not pts:
        body = (
            '<text x="450" y="260" text-anchor="middle" font-size="16">'
            "No successful runs with actual memory metrics."
            "</text>"
        )
    ...
    sx = scale(xs, lo, hi, MARGIN, X_MAX)
    sy = scale(ys, lo, hi, Y_MAX, MARGIN)

    body = [
        f'<line x1="{MARGIN}" y1="{Y_MAX}" x2="{X_MAX}" y2="{MARGIN}"'
        ' stroke="#888" stroke-dasharray="6,4"/>',
        f'<line x1="{MARGIN}" y1="{Y_MAX}" x2="{X_MAX}" y2="{Y_MAX}" stroke="black"/>',
        f'<line x1="{MARGIN}" y1="{Y_MAX}" x2="{MARGIN}" y2="{MARGIN}" stroke="black"/>',
    ]
...
def throughput_error_bars(rows: list[dict[str, str]], output: Path) -> None:
    ...
    if not vals:
        body = (
            '<text x="450" y="260" text-anchor="middle" font-size="16">'
            "No successful runs with actual throughput metrics."
            "</text>"
        )
...
def calibration(rows: list[dict[str, str]], output: Path) -> None:
    ...
    y = Y_MAX - observed * (HEIGHT - 2 * MARGIN)
    body = "\n".join(
        [
            f'<line x1="{MARGIN}" y1="{Y_MAX}" x2="{X_MAX}" y2="{MARGIN}"'
            ' stroke="#888" stroke-dasharray="6,4"/>',
            ...
            f'font-size="12">({pred_prob:.2f}, {observed:.2f})</text>',
        ]
    )
...
def break_even(rows: list[dict[str, str]], output: Path) -> None:
    ...
    if not preds:
        body = (
            '<text x="450" y="260" text-anchor="middle" font-size="16">'
            "No predicted break-even values available."
            "</text>"
        )
    ...
    for i, p in enumerate(preds):
        x = MARGIN + spacing * (i + 0.5)
        y = Y_MAX - (p / max_y) * (HEIGHT - 2 * MARGIN)
    ...
    if all(a is None for a in actuals):
        body.append(
            '<text x="450" y="470" text-anchor="middle" font-size="14">'
            "Actual break-even unavailable (all runs failed)."
            "</text>"
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--input",
        default="validation_results/validation_2026_04/validation_runs.csv",
    )
