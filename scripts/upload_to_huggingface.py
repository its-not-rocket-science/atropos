...
def upload_model(
    ...
    if org_name:
        result.repo_id = f"{org_name}/{repo_name}"
    else:
        result.repo_id = repo_name
...
def upload_all_models(
    ...
    """Upload all pruned models to HuggingFace.
    ...
    model_dirs = sorted([d for d in pruned_dir.iterdir() if d.is_dir()])
